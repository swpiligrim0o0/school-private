...
@section('title', 'Yangi Davomat')

@section('content')
<div class="row">
    <div class="col-md-8 offset-md-2">
        <h2>➕ Yangi Davomat Qo'shish</h2>
        <hr>
        ...
        <form action="{{ route('attendance.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-control @error('status') is-invalid @enderror" id="status" name="status" required>
                    <option value="present" {{ old('status') == 'present' ? 'selected' : '' }}>✅ Keldi</option>
                    <option value="absent" {{ old('status') == 'absent' ? 'selected' : '' }}>❌ Kelmadi</option>
                    <option value="late" {{ old('status') == 'late' ? 'selected' : '' }}>⏰ Kech Keldi</option>
                </select>
            </div>
